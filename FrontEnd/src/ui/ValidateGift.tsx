...
const ValidateGift: React.FC<{ walletAddress: string }> = ({ walletAddress }) => {
  ...
  const validateGift = async (rawCode: string): Promise<GiftDetails> => {
    try {
      const response = await axios.post('http://localhost:3001/api/gifts/validate', {
        code: rawCode,
        walletAddress
      });
  ...
  const handleClaimGift = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3001/api/gifts/claim', {
        code,
        walletAddress
      });
